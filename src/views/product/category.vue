        ...
        <el-table-column label="导航状态" width="100" align="center">
          <template #default="{ row }">
            <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              @change="handleNavStatusChange(row, $event)"
            />
          </template>
        </el-table-column>
        <el-table-column label="显示状态" width="100" align="center">
          <template #default="{ row }">
            <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              @change="handleShowStatusChange(row, $event)"
            />
          </template>
        </el-table-column>
...
const handleDelete = async (row: CategoryTreeItem) => {
  try {
    await ElMessageBox.confirm(`确定要删除分类"${row.name}"吗？删除后无法恢复！`, '警告', {
    ...
    const error = _error as { response?: { data?: { message?: string } }; message?: string }
    if (error !== 'cancel') {
...
// 导航状态切换
const handleNavStatusChange = async (row: CategoryTreeItem, newVal: number) => {
  if (!dataInitialized.value) return
  const oldVal = row.navStatus === 1 ? 0 : 1
  try {
    ...
    row.navStatus = oldVal
    ElMessage.error('更新失败')
  }
}

// 显示状态切换
const handleShowStatusChange = async (row: CategoryTreeItem, newVal: number) => {
  if (!dataInitialized.value) return
  const oldVal = row.showStatus === 1 ? 0 : 1
  try {
    await updateCategory(row.id, {
      name: row.name,
      parentId: row.parentId,
